...
output_path = '/Users/xw0201/Desktop/postdoc/CPI_structure_CALB/Large_Dataset/allec/KCAT/brenda_ec'
...
password = ''
#6) Information to retrieve: km, M.W., pathway, sequence, specific activity and kcat.
# fields = ['KM','KcatKm','KCAT']
fields = ['KCAT']
# require PDB (1) or not (0)
pdb_require = 1

################################################################################

# extract_field: Function that extracts all BRENDA data from a specific field.
def extract_field(field, last, end):
    ...
    if field == 'KM':
        ...
        ECstring = client.getEcNumbersFromKcatKmValue(credentials)
            ...
            while succes < 10:
                try:
                    ...
                    if field == 'KM':
                        ...
                        data = client.getKcatKmValue(query)
...
for field in fields:
    ...
    if start:
        extract_field(field, last_EC, end_EC)
