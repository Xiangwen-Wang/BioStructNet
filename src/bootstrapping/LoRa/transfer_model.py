...
class MoRALayer(nn.Module):
    def __init__(self, in_features, rank):
        super(MoRALayer, self).__init__()
        self.rank = rank
        self.A = nn.Parameter(torch.randn(in_features, rank))
        self.B = nn.Parameter(torch.randn(rank, in_features))

    def forward(self, x):
        seq_len, batch_size, in_features = x.size()
        low_rank_adjustment = torch.matmul(self.A, self.B)  # [in_features, in_features]
        low_rank_adjustment = low_rank_adjustment.unsqueeze(0).unsqueeze(0)  # [1, 1, in_features, in_features]
        low_rank_adjustment = low_rank_adjustment.expand(seq_len, batch_size, -1, -1)  # [seq_len, batch_size, in_features, in_features]
        adjusted_x = torch.einsum('sbi,sbij->sbi', x, low_rank_adjustment)  # [seq_len, batch_size, in_features]
        output = adjusted_x + x
        return output
...
class BANLayer(nn.Module):
    def __init__(self, v_dim, q_dim, h_dim, h_out, act='ReLU', dropout=0.2, k=3):
        ...
        self.mora = MoRALayer(h_dim * self.k, rank=16)
    ...
    def forward(self, v, q, softmax=False):
        ...
        v_ = self.mora(v_.transpose(0, 1)).transpose(0, 1)
        q_ = self.mora(q_.transpose(0, 1)).transpose(0, 1)
...
class TransferLearningModel(nn.Module):
    def __init__(self, source_model, **config):
        ...
        mora_started = False
        for name, param in self.bcn.named_parameters():
            if 'mora' in name:
                mora_started = True

            if not mora_started:
                source_param = dict(self.source_model.bcn.named_parameters())[name]
...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            train_loss, train_accuracy = self.train_epoch()  # 更新train_epoch以返回准确率
